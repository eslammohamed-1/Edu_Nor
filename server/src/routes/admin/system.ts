...
import { errorSchema, okTrueSchema, publicUserSchema } from '../../lib/openapi-schemas.js';
...
export const adminSystemRoutes: FastifyPluginAsync<{ env: Env }> = async (app, { env }) => {
  ...
  app.put(
    '/snapshots/:key',
    {
      schema: {
        params: { type: 'object', properties: { key: { type: 'string' } }, required: ['key'] },
        body: { type: 'object', required: ['data'], properties: { data: {} }, additionalProperties: false },
        response: {
          200: {
            type: 'object',
            properties: { ok: { type: 'boolean' }, updatedAt: { type: 'string' } },
      ...
      schema: {
        ...
        response: {
          200: {
            ...
            required: ['user', 'accessToken', 'impersonatedBy']
          },
          400: errorSchema,
          404: errorSchema
        }
